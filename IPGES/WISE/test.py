...
def performRMS(samples):
	
	rms = 0
	for i in range(len(samples)):
		rms = rms + np.square(samples[i])
...
def loaddata(pom, samples):
	...
	for sample in sample_arr:
		...
		max.append(np.amax(sample))
		min.append(np.amin(sample))
		fft.append(performFFT(sample))
		rms.append(performRMS(sample))
